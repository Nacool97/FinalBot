...
class TelegramBot:
    ...
    def action(self):
        ...
        if self.incoming_message_text == '/hello':
            self.outgoing_message_text = "Hello {} {}.\nI'm a Alton, how may I help you?".format(self.first_name)
        ...
        if self.incoming_message_text.find('/dict')==0:

            dictionary = PyDictionary()
            query = self.incoming_message_text.replace("/dict","")
            if query=='':
                self.outgoing_message_text = 'Invalid query'
            else:
                result = dictionary.meaning(query)
                synon = dictionary.synonym(query)
                self.outgoing_message_text = '{}\n{}'.format(result,synon)
            success = self.send_message()
